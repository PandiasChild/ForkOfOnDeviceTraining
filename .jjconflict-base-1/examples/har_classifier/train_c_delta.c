#define SOURCE_FILE "har_classifier_train_c_delta"

/* DELTA-quantized-WEIGHTS HAR conv classifier (de-risking a config the
 * framework has never run: trainable params stored as packed sub-byte SYM).
 *
 * Config (see .superpowers/sdd/sym-spike-report.md):
 *   - weight storage + bias storage = packed DELTA@DELTA_BITS (the sub-byte `DELTA`
 *     qtype, NOT the SYM_INT32 int32 container). Factories can only init
 *     FLOAT32-native param storage (#270 requireFloat32 gate), so the params
 *     are built FLOAT32/Kaiming and requantized in place to DELTA@DELTA_BITS after
 *     construction (requantizeTensorInPlace — the mixed_width_mlp dance,
 *     retargeted from SYM_INT32 to packed DELTA).
 ...
 *     DELTA weight/bias that is conversionMatrix[DELTA][SYM_INT32]
 *     (convertSymQDeltaTensorToSymInt32Tensor); for the FLOAT32 activation wire it is
 ...
 *     dispatches on the ACTUAL param dtype, so the DELTA param round-trips
 *     DELTA->FLOAT32->DELTA (both cells exist); the "SYM_INT32" tag is a misnomer
 *     for "quantized param, dequant-update-requant".
 *
 * SYM_BITS / DELTA_BITS/ SEED / LR / MOMENTUM / EPOCHS / LOG_PATH are env-overridable. */
#include <errno.h>
#include <math.h>
#include <stdint.h>
#include <stdio.h>
...
typedef struct quantTemplate {
    ...
    quantization_t
        *symOrDeltaQ; /* packed SYM@SYM_BITS OR DELTA@DELTA_BITS — post-init weight+bias storage */
} quantTemplate_t;
 ...
 */
static layerQuant_t createLayerQuant(quantTemplate_t *qt) {
    return (layerQuant_t){
        ...
        .outputQ = qt->floatQ,
        .propLossQ = qt->floatQ,
        .weightStorage = qt->floatQ, /* temp; requantized to DELTA@DELTA_BITS post-build */
        .biasStorage = qt->floatQ,   /* temp; requantized to DELTA@DELTA_BITS post-build */
...
static void buildModel(layer_t **model, quantTemplate_t *sq, layerQuant_t *lqFloat) {
    layerQuant_t lqSym = createLayerQuant(sq);
 ...
 * knob at construction).
 */
static void requantizeParamsToDelta(layer_t **model, quantTemplate_t *dq) {
    const size_t convIdx[3] = {0, 3, 6};
    for (size_t k = 0; k < 3; k++) {
        conv1dConfig_t *cfg = model[convIdx[k]]->config->conv1d;
        requantizeTensorInPlace(cfg->weights->param, dq->symOrDeltaQ);
        if (cfg->bias != NULL) {
            requantizeTensorInPlace(cfg->bias->param, dq->symOrDeltaQ);
        }
    }
    linearConfig_t *fc = model[10]->config->linear;
    requantizeTensorInPlace(fc->weights->param, dq->symOrDeltaQ);
    requantizeTensorInPlace(fc->bias->param, dq->symOrDeltaQ);
}
...
/* PARAM gate: every trainable weight/bias tensor is packed DELTA@DELTA_BITS.
 * GRAD gate: every trainable grad tensor is FLOAT32 (the NULL grad-knob default).
 */
...
int main(int argc, char *argv[]) {
     ...
     */
    if (g_deltaBits < 1 || g_deltaBits > 12) {
        fprintf(stderr,
                "DELTA_BITS must be in [1, 12]: the SYM_INT32 forward operand contract caps at 12 "
    ...
    fprintf(
        stdout,
        "CONFIG deltabits=%d qBits=%d lr=%.5f momentum=%.3f epochs=%d seed=%u shuffle_seed=%u\n",
        g_deltaBits, g_symBits, (double)g_lr, (double)g_momentum, g_epochs, g_seed, g_shuffleSeed);
     ...
     * optimizer epic's dead-zone escape (#279): a FLOAT32 grad step smaller than one SYM
     * level would deterministically round back to the same int under HALF_AWAY and vanish;
     * SR_HALF_AWAY rounds it up with probability equal to the fractional part, so sub-ULP
     * updates move the weight IN EXPECTATION.
    ...
    quantTemplate_t sq = {
        ...
        .symOrDeltaQ = quantizationInitSymQDelta(g_symBits, symRounding, g_deltaBits),
    };
    ...
    paramGateCtx_t wCtx = {.isGrad = false,
                           .expectBits = g_symBits,
                           .expectDeltaBits = g_deltaBits,
                           .count = 0,
                           .fails = 0};
 ...
 * dequant->float-update->requant path. convertTensor dispatches on each tensor's ACTUAL           \
 * dtype: the packed-SYM weights round-trip SYM<->FLOAT32 per step (write-back stochastic          \
 * via symRounding), while the FLOAT32 grad and the FLOAT32 momentum state (below) are             \
 ...
 */
     ...
     * the epic's own-config knob. A packed-SYM momentum would requantize the running velocity
     * to the same coarse levels as the weights and reinstate a dead-zone in the accumulator;
