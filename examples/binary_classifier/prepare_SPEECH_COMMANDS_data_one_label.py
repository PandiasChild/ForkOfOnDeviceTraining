...
sample rate: 16000, each audio 1 second
Output:
...
ZIP_URL = "http://download.tensorflow.org/data/speech_commands_v0.02.tar.gz"
ARCHIVE_PATH = RAW_DIR / "speech_commands_v0.02.tar.gz"
EXTRACTED_DIR = RAW_DIR / "speech_commands_v0.02"
NPY_DIRECTORY = DATA_DIR / "SPEECH_COMMANDS_one_label"

def _load_dataset() -> tuple[np.ndarray, np.ndarray]:
    X, y = [], []
    yes_directory = EXTRACTED_DIR / "yes"
    no_directory = EXTRACTED_DIR / "no"
    yes_wav_files = sorted(yes_directory.rglob("*.wav"))
    no_wav_files = sorted(no_directory.rglob("*.wav"))

    for folder in [yes_wav_files, no_wav_files]:
        for f in folder:
            sr, audio = read_wav(f)
            ...
            parent_folder_name = f.parent.name
            if parent_folder_name == "yes":
                label = 1
            elif parent_folder_name == "no":
                label = 0
