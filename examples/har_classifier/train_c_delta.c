...
static layerQuant_t createLayerQuant(quantTemplate_t *qt) {
    quantization_t *wireQ = g_symWires ? qt->symWireQ : qt->floatQ;
    arithmetic_t dxMath = g_symWires ? (arithmetic_t){ARITH_SYM_INT32, HALF_AWAY} : (arithmetic_t){ARITH_FLOAT32, HALF_AWAY};
    return (layerQuant_t){
...
static void buildModel(layer_t **model, quantTemplate_t *sq, layerQuant_t *lqNoTrain) {
    ...
    model[0] = conv1dLayerInit(
        ...
        &lqSym);
    model[1] = reluLayerInit(lqNoTrain);
    ...
    model[3] = conv1dLayerInit(
        ...
        &lqSym);
    model[4] = reluLayerInit(lqNoTrain);
    ...
    model[6] = conv1dLayerInit(
        ...
        &lqSym);
    model[7] = reluLayerInit(lqNoTrain);
    model[8] = avgPool1dLayerInit(
        &(avgPool1dInit_t){.kernelSize = LEN_INPUT / 4, .stride = LEN_INPUT / 4}, lqNoTrain);
